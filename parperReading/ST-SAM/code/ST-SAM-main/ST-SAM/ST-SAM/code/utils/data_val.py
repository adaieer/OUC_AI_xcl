...
aug = A.Compose([
    ...
    A.VerticalFlip(p=0.5),    # 以 50% 的概率垂直翻转
    A.RandomRotate90(p=0.5),  # 以 50% 的概率随机旋转 90 度
    A.GaussNoise(p=0.5),
    A.Blur(p=0.2),
...
class PolypObjDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load edge if in training mode
        if self.istraining:
            edge = self.binary_loader(self.edges[index])
            if edge.size != image.size:
                raise ValueError(f"Image and edge size mismatch for {name}: image={image.size}, edge={edge.size}\n"
                               f"Image path: {self.images[index]}\n"
                               f"Edge path: {self.edges[index]}")
