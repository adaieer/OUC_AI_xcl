...
class Discriminator(nn.Module):
	...
	def __init__(self):
		...
		layers.append(nn.LeakyReLU(0.2, inplace=True))  # 改进的ReLU，当输入小于0时，输出为0.2*输入值，否则输出为输入。
		# 第二层
...
for epoch in range(total_epochs):
	...
	fixed_fake_images = generator(fixed_z)  # 生成100张图片
	save_image(fixed_fake_images, 'gan_images/{}.png'.format(epoch), nrow=10, normalize=True) # 每行10张保存
